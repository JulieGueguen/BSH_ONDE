...
La Xème campagne usuelle d’observation du réseau ONDE pour l’année `r annee_campagne` s'est déroulée au cours de la semaine `r num_semaine` (du `r format(min(unique(onde_DC_usuelles$date_campagne), na.rm = TRUE), "%d/%m/%Y")` au `r format(max(unique(onde_DC_usuelles$date_campagne), na.rm = TRUE), "%d/%m/%Y")`).
...
tab_mois_courant_evolution %>%
  ...
  pivot_wider(names_from = code_departement,
    ...
    autofit() %>%
    theme_zebra() %>%
  set_caption(caption = paste0("Evolution de l'écoulement par rapport au mois précédent en fonction des départements - Situation au ", date_campagne))
...
tab_mois_courant_evolution %>%
  ...
  autofit() %>%
  theme_zebra() %>%
  set_caption(caption = paste0("Evolution de l'écoulement par rapport au mois précédent en fonction des bassins - Situation au ", date_campagne))
...
```{r sankey,  fig.width=10, fig.height=12, fig.cap=paste0("Diagramme des écoulements - Campagne usuelle ", mois_camapgne, annee_campagne)}
...
ggplot(data = data_alluvial_freq2,
  ...
  ggplot2::labs(#x = paste("Mois", moisAVoir, "et le mois précédent"),
                y = "Classes d'écoulements",
                  ...
                  title = paste("Classements des écoulements entre le mois d'", 
...
```{r indice, fig.width=10, fig.height=6, fig.cap=paste0("Notes d'indices pour la région Grand-Est - Campagne usuelle ", mois_camapgne, annee_campagne)}
...
```{r median 2012_2023, fig.cap=paste0("Diagramme présentant les situations d'écoulement pour la région Grand-Est - Campagne usuelle ", mois_camapgne, "(2012 - ", annee_campagne, ")")}
