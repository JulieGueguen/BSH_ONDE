...
# ajout
if(!require("ondetools")) install.packages('ondetools')
```
...
# Nombre de stations ONDE dans le département `r params$codes_dpt`

```{r, include=FALSE, echo=FALSE}

### prétraitement sur le département
nb_stations <- params$onde_df2 %>% 
  dplyr::filter(code_departement %in% params$codes_dpt & libelle_type_campagne == 'usuelle') %>% 
...
date_derniere_campagne_usuelle <- params$onde_df2 %>% 
  dplyr::filter(libelle_type_campagne == 'usuelle') %>% 
  dplyr::filter(code_departement %in% params$codes_dpt) %>% 
  dplyr::mutate(code_campagne = as.numeric(code_campagne)) %>% 
  dplyr::filter(code_campagne == max(code_campagne)) %>% 
...
date_derniere_campagne_compl <- params$onde_df2 %>% 
...
mois_sel <- format(lubridate::ym(params$annee_mois), "%m")
annee_sel <- format(lubridate::ym(params$annee_mois), "%Y")
...
onde_bilan_ecoulement_annee_nat <-
  ...
  ondetools::calculer_bilan_ecoulement(onde_df = .,
                            mod = lib_ecoul,
...
Actuellement, le **département `r params$codes_dpt`** est couvert par **`r nb_stations` stations** d'observations. 
...
Les cartes ci-dessous présentent les situations observées sur les stations du **département `r params$codes_dpt`** pour la campagne **`r params$type_rapport` du mois de `r params$mois_rapport`**. 
...
produire_carte_statique(code_departement = params$codes_dpt,
...
produire_graph_type_ecoulement(data_bilan = onde_bilan_ecoulement_annee_nat,
                               ...
                               historique = F) +
  ggplot2::labs(title= glue::glue("Types d'écoulement de l'année - département {params$codes_dpt}")) + ggplot2::theme(legend.position = 'none')
...
produire_graph_type_ecoulement(data_bilan = onde_bilan_ecoulement_annee_nat,
                               lib_ecoulement = lib_ecoul,
                               historique = F) +
  ggplot2::labs(title= glue::glue("Types d'écoulement de l'année - département {params$codes_dpt}")) +
